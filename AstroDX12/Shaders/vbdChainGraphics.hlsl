...
PSInput VS(uint VertexID : SV_VertexID, uint InstanceID : SV_InstanceID)
{
    ...
    StructuredBuffer<VBDChainElementData> chainElementData = ResourceDescriptorHeap[chainElementBufferIndex];
    ...
    o.Color = chainElementData[InstanceID].Pinned ? float4(1.f, 0.f, 0.f, 1.f) : float4(0.5f, 0.2f, 1.f, 1.f);
